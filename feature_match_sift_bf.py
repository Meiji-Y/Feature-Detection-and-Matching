...
def get_camera_poses(frames, intrinsic_matrix, init_camera_pose):
    ...
    for i in range(1, len(frames)):
        ...
        matches = bf_matcher.knnMatch(prev_descriptors, curr_descriptors, k=2)
        ...
        prev_matched_pts = np.float32(
            [prev_keypoints[match.queryIdx].pt for match in good_matches]
        ).reshape(-1, 1, 2)
        curr_matched_pts = np.float32(
            [curr_keypoints[match.trainIdx].pt for match in good_matches]
        ).reshape(-1, 1, 2)
...
K = np.array([[666.8, 0, 309.26], [0, 671.15, 247.67], [0, 0, 1]])
